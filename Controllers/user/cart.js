...
const addTOCart = async (req, res) => {
  try {
    ...
    await User.updateOne(
      { _id: userId, "cart.items": { $elemMatch: { productId: id } } },
      { $set: { "cart.totalItems": cartLenght.length } }
    ); 
  } catch (error) {
    console.log(error);
...
const viewCart = async (req, res) => {
  try {
    ...
    const cart = await User.findOne({ _id: userId }).populate(
    ...
    const price = await User.findOne({ _id: userId }).populate(
      "cart.items.price"
    );
    const items = await User.findOne({ _id: userId }).populate(
      "cart.totalItems"
    );
    const total = await User.findOne({ _id: userId }).populate(
      "cart.totalPrice"
    );
...
const deleteCart = async (req, res) => {
  try {
    ...
    const items = await User.aggregate([
      { $match: { _id: mongoose.Types.ObjectId(userId) } },
      { $unwind: "$cart.items" },
      { $group: { _id: "$cart.itmes", totalItems: { $sum: "$cart.items" } } },
    ]);
    const price = await User.aggregate([
      ...
      { $unwind: "$cart.items" },
      {
        $group: {
          _id: "$cart.itmes",
          totalPrice: { $sum: "$cart.items.price" },
        },
      },
    ]);
    if(price.length!=0){
        await User.updateOne(
            { _id: userId },
            { $set: { "cart.totalPrice": price[0].totalPrice } }
          );
    }else{
        await User.updateOne(
            { _id: userId },
